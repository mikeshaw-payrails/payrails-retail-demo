...
const Checkout = () => {
  ...
  const { convertPrice } = useLocationDetection(); // <-- add
  const currency = selectedLocation?.currency || "USD";

  // Convert the total from USD to selected currency (prices are defined in USD)
  const localizedTotal = selectedLocation
    ? convertPrice(total, "USD", selectedLocation.currency)
    : total;

  const handlePaymentSuccess = (data: { orderNumber: string }) => {
  ...
  const renderPaymentIntegration = () => {
    switch (activeIntegration) {
      case "dropin":
        return <DropInIntegration total={localizedTotal} currency={currency} onSuccess={handlePaymentSuccess} />; // <-- changed
      case "elements":
        return <ElementsIntegration total={localizedTotal} currency={currency} onSuccess={handlePaymentSuccess} />;
      case "secure-fields":
        return <SecureFieldsIntegration onSuccess={handlePaymentSuccess} />;
      default:
        return <DropInIntegration total={localizedTotal} currency={currency} onSuccess={handlePaymentSuccess} />; // <-- changed
    }
  };
